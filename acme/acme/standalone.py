    ...
    def __init__(self, ServerClass, server_address, *remaining_args, **kwargs):
        ...
        for ip_version in [True, False]:
            try:
                ...
                logger.debug("Successfully bound to %s:%s using %s", new_address[0],
                    new_address[1], "IPv6" if ip_version else "IPv4")
            except socket.error:
                if self.servers:
                    # Already bound using IPv6.
                    logger.debug("Failed to bind to %s:%s using %s, but continuing as we've bound using %s",           new_address[0], new_address[1], "IPv6" if ip_version else "IPv4", "IPv6")
                else:
