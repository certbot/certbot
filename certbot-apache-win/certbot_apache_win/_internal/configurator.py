...
from certbot_apache_win._internal import assertions
from certbot_apache_win._internal import constants
from certbot_apache_win._internal import display_ops
from certbot_apache_win._internal import dualparser
from certbot_apache_win._internal import http_01
from certbot_apache_win._internal import obj
from certbot_apache_win._internal import parser
from certbot_apache_win._internal import winapacheparser
...
class ApacheConfigurator(common.Installer):
    ...
    :type parser: :class:`~certbot_apache_win._internal.parser`

    :ivar tup version: version of Apache
    :ivar list vhosts: All vhosts found in the configuration
        (:class:`list` of :class:`~certbot_apache_win._internal.obj.VirtualHost`)
    ...
    def prepare(self):
        ...
        self.apacheCtl = self.option("ctl").replace("\\","/")
        print("ApacheCtl:{0}".format(self.apacheCtl))
        
        self.filePath = os.path.abspath("{0}/{1}".format(os.path.dirname(self.apacheCtl),"../conf/httpd.conf")).replace("\\","/")
        self.basepath = os.path.abspath("{0}/{1}".format(os.path.dirname(self.apacheCtl),"../")).replace("\\","/")


        print("FilePath:{0}".format(self.filePath))
        print("BasePath:{0}".format(self.basepath))
        ...
        self.parser = winapacheparser.WinApacheParser(self.basepath)
        self.nodes = set()
        ...
        #TODO fetch virtual hosts with parser
    ...
    def revert_challenge_config(self):
        ...
        self.parser.revert_challenge(self.filePath)
    ...
    def choose_vhosts(self, domain, create_if_no_ssl=True):
        ...
        :rtype: `list` of :class:`~certbot_apache_win._internal.obj.VirtualHost`
        """
    ...
    def _deploy_cert(self, vhost, cert_path, key_path, chain_path, fullchain_path):
        ...
        self._add_dummy_ssl_directives(vhost)
        ...
        logger.info("Deploying Certificate to VirtualHost in %s at line:%d", vhost.node.filePath,vhost.node.startLine)
        ...
        if self.version < (2, 4, 8) or (chain_path and not fullchain_path):
            ...
            if chain_path is not None:
                self.parser.update_directive(vhost,
    ...
    def choose_vhost(self, target_name, create_if_no_ssl=True):
        ...
        :rtype: :class:`~certbot_apache_win._internal.obj.VirtualHost`
    ...
    def _choose_vhost_from_list(self, target_name, temp=False):
        ...
        if not vhost.ssl:
            ...
            print("enabled:{0}".format(vhost.enabled))
            if not (vhost.enabled and vhost.conflicts(addrs,self.vhosts)):
                vhost = self.make_vhost_ssl(vhost)
            else:
                logger.error(
    ...
    def _find_best_vhost(self, target_name, vhosts=None, filter_defaults=True):
        ...
        :type vhosts: `collections.Iterable` of :class:`~certbot_apache_win._internal.obj.VirtualHost`
        ...
        if best_candidate is None:
            if filter_defaults:
                myvhosts = self._non_default_vhosts(vhosts)
            # remove mod_macro hosts from reasonable vhosts
            reasonable_vhosts = [vh for vh
                                 in myvhosts if vh.modmacro is False]
    ...
    def _get_vhost_names(self, vhostNode):
        ...
        serveraliases = set()
        servername = None 
        for child in vhostNode.node.children:
            name = child.name.strip().casefold()
            if name == 'serveralias'.casefold():
                serveraliases.update(set(child.content.strip().split()))
            
            if name == 'servername'.casefold():
                servername = child.content.strip().split()
    ...
    def _add_servernames(self, host):
        ...
        :type host: :class:`~certbot_apache_win._internal.obj.VirtualHost`
    ...
    def get_virtual_hosts(self):
        ...
        for node in self.nodes:
            v1_vhosts.extend(self.get_virtual_hosts_v1(node))
       
        print("Length of vhost:{0}".format(len(v1_vhosts)))
        return v1_vhosts

    def get_virtual_hosts_v1(self,currentNode):
        """Returns list of virtual hosts found in the Apache configuration.

        :returns: List of :class:`~certbot_apache_win._internal.obj.VirtualHost`
        ...
        for node in currentNode.children:
            if node.nodeType == winapacheparser.Type.TAG and node.name.casefold() == "virtualhost".casefold():
                virtualHosts.append(node)

        vhs = []
        for vhost in virtualHosts:
            print("virtual host:{0} Addr:{1}".format(vhost.name,vhost.content))
            addrs = obj.Addr.fromstring(vhost.content.strip())
            is_ssl = False
            ...
            for child in vhost.children:
                name = child.name.strip().casefold() 
                if name == 'servername'.casefold():
                    vhostName = child.content.strip()

                if name == 'serveralias'.casefold():
                    vhostAlias.update(set(child.content.strip().split()))

                if name == 'SSLCertificateFile'.casefold():
                    is_ssl = True

            print("ServerName:{1} Vhost Aliases are:{0}".format(vhostAlias,vhostName))
            vhostObj = obj.VirtualHost(
            vhost.filePath, None, addrs, is_ssl, True,name=vhostName,aliases=vhostAlias, modmacro=False, node=vhost)
    ...
    def get_virtual_hosts_v2(self):
        ...
        :returns: List of :class:`~certbot_apache_win.obj.VirtualHost`
            objects found in configuration
        :rtype: list
        """
    ...
    def _create_vhost_v2(self, node):
        ...
        :returns: newly created vhost
        :rtype: :class:`~certbot_apache_win.obj.VirtualHost`
        """
        addrs = set()
    ...
    def _populate_vhost_names_v2(self, vhost):
        ...
        :param host: In progress vhost whose names will be added
        :type host: :class:`~certbot_apache_win.obj.VirtualHost`
        """
    ...
    def is_name_vhost(self, target_addr):
        ...
        :param certbot_apache_win._internal.obj.Addr target_addr: vhost address
    ...
    def add_name_vhost(self, addr):
        ...
        :type addr: :class:`~certbot_apache_win._internal.obj.Addr`
    ...
    def ensure_listen(self, port, https=False):
        ...
        for node in self.nodes:
            self.parser.findListenDirectives(listenDirectives,node)
    ...
    def make_vhost_ssl(self, nonssl_vhost):
        ...
        :type nonssl_vhost: :class:`~certbot_apache_win._internal.obj.VirtualHost`
        ...
        #self._add_dummy_ssl_directives(nonssl_vhost)
        serverName=None
        ssl_vhost = None
        for child in nonssl_vhost.node.children:
            if child.name.strip().casefold()=="ServerName".casefold():
                serverName=child.content.strip()


        self.parser.add_dir_raw(nonssl_vhost,"SSLProtocol","-all +TLSv1.2")
        self.parser.add_dir_beginning(nonssl_vhost,"SSLEngine","on")
        addLsten = False
        listenDirectives = set()
        for node in self.nodes:
            self.parser.findListenDirectives(listenDirectives,node)
        ...
        listens = map(lambda x:x.content.strip(),listenDirectives)
        for listen in listens:
            if not (listen.find("443") >-1 or listen.find("{0}:{1}".format(nonssl_vhost.addrs.get_addr(),nonssl_vhost.addrs.get_port()))>-1):
                addLsten=True
        addDummyVhost = True
        for vhost in self.vhosts:
            if(vhost.node.content.strip()=="*:80"):
                addDummyVhost = False

        if nonssl_vhost.addrs.get_port() == "80":                
            self.parser.update_virtualhost_address(nonssl_vhost,"443",addLsten,addDummyVhost)

        print("Searching for servername:{0}".format(serverName))
        self.nodes = set()
        self.parser.findNestedNodes(self.nodes,self.filePath)
        self.vhosts = self.get_virtual_hosts()
        time.sleep(5)
        print("Making vhost count:{0}".format(len(self.vhosts)))
        for vhost in self.vhosts:
            for child in vhost.node.children:
                if child.name.strip().casefold()=="ServerName".casefold() and child.content.find(serverName)>-1:
                    print("Vhost:{0}".format(child.content))
                    print("Start:{0} End:{1}".format(vhost.node.startLine,vhost.node.endLine))
                    print("vhostdata:{0}".format(vhost.node.content))
                    ssl_vhost = vhost
            if ssl_vhost:
                break
    ...
    def _add_dummy_ssl_directives(self, virtualHost):
        ...
        for child in virtualHost.node.children:
            ...
            if name == "SSLCertificateKeyFile".casefold():
                add_certificate_key = False

        if add_certificate:
            self.parser.add_dir(virtualHost, "SSLCertificateFile","insert_cert_file_path")
        if add_certificate_key:
    ...
    def _add_servername_alias(self, target_name, vhost):
        ...
        if target_name == sname or target_name in saliases:
            return
        if self._has_matching_wildcard(vhost, target_name):
            return
        if 'ServerName' in list(map(lambda k: k.name,vhost.node.children)) and 'ServerAlias' not in list(map(lambda k: k.name,vhost.node.children)):
            self.parser.add_dir(vhost,"ServerAlias",' '.join(str(e) for e in vhost.get_names()))
        
        self._add_servernames(vhost)

    def _has_matching_wildcard(self, vhost, target_name):
        ...
        for child in vhost.node.children:
            name = child.name.strip().casefold()
            if name == 'serveralias'.casefold():
                aliases = child.content.strip().split()
    ...
    def _add_name_vhost_if_necessary(self, vhost):
        ...
        :type vhost: :class:`~certbot_apache_win._internal.obj.VirtualHost`
    ...
    def find_vhost_by_id(self, id_str):
        ...
        :rtype: :class:`~certbot_apache_win._internal.obj.VirtualHost` or None
    ...
    def _enable_ocsp_stapling(self, ssl_vhost, unused_options):
        ...
        :type ssl_vhost: :class:`~certbot_apache_win._internal.obj.VirtualHost`

        :param unused_options: Not currently used
        :type unused_options: Not Available

        :returns: Success, general_vhost (HTTP vhost)
        :rtype: (bool, :class:`~certbot_apache_win._internal.obj.VirtualHost`)
    ...
    def _create_redirect_vhost(self, ssl_vhost):
        ...
        :returns: tuple of the form
            (`success`, :class:`~certbot_apache_win._internal.obj.VirtualHost`)
        :rtype: tuple
    ...
    def _get_proposed_addrs(self, vhost, port="80"):
        ...
        """
        redirects = set()
        redirects.add(vhost.addrs.get_addr_obj(port))
    ...
    def _enable_autohsts_domain(self, ssl_vhost):
        ...
        :type ssl_vhost: :class:`~certbot_apache_win._internal.obj.VirtualHost` or None
