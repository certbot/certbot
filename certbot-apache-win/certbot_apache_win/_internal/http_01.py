...
from certbot_apache_win._internal.obj import VirtualHost  # pylint: disable=unused-import
from certbot_apache_win._internal.parser import get_aug_path
...
class ApacheHttp01(common.ChallengePerformer):
    ...
    def _mod_config(self):
        ...
        for vhost in selected_vhosts:
            if vhost.addrs.is_wildcard() or vhost.addrs.get_port() == http_port:
                found = True
    ...
    def _matching_vhosts(self, domain):
        ...
        if len(self.configurator.vhosts)>0:
            for vhost in self.configurator.vhosts:
                for node in vhost.node.children:
                    if ( node.name.casefold() == 'servername'.casefold() or node.name.casefold() == 'serveralias'.casefold() ) and domain.casefold() in node.content.strip().casefold() :
                        logger.info("node start:{0} end:{1}".format(node.startLine,node.endLine))
    ...
    def _relevant_vhosts(self):
        ...
        relevant_vhosts = []
        for vhost in self.configurator.vhosts:
            if vhost.addrs.is_wildcard() or vhost.addrs.get_port() == http01_port:
                if not vhost.ssl:
    ...
    def _set_up_include_directives(self, vhost):
        ...
        if vhost not in self.moded_vhosts:
            ...
            self.configurator.parser.add_dir_beginning(
                vhost, "Include", self.challenge_conf_pre)
            self.configurator.parser.add_dir(
                vhost, "Include", self.challenge_conf_post)
