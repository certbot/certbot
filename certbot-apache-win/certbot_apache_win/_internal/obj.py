...
class Addr(common.Addr):
    ...
    def __repr__(self):
        return "certbot_apache_win._internal.obj.Addr(" + repr(self.tup) + ")"
...
class VirtualHost(object):
    ...
    def __hash__(self):
        return hash((self.filep, self.path,
                     self.addrs, tuple(self.get_names()),
                     self.ssl, self.enabled, self.modmacro))

    def conflicts(self, addrs,vhosts):
        ...
        for vhost in vhosts[0]:
            if vhost.addrs.conflicts(addrs):
                return True
        return False
