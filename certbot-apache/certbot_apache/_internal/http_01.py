...
class ApacheHttp01(common.ChallengePerformer):
    ...
    def _mod_config(self):
        """
        This function modifies the Apache configuration for challenges by adding
        a challenge directory to the web server's configuration.

        :param list[str]
        vhosts: The VirtualHosts that are being enabled for
            challenge handling.
    ...
    def _relevant_vhosts(self):
        """
        .. autofunc :: acme_tiny.challenges._relevant_vhosts
           :module: acme_tiny.challenges
           :private:

            Finds a virtual host that listens on port
        http01_port and is currently not serving HTTPS, so it can be used to prove control of the domain to the CA. If no such vhost is found, raises
        PluginError with helpful message explaining what needs to be done so Certbot can help with proving control of the domain for challenges.

            :param
        list relevant_vhosts: Virtual hosts Certbot has found listening on port http01_port (and not serving HTTPS).
            :returns: A single virtual host that
        listens on port http01_port and is currently not serving HTTPS (so it can be used to prove control of a domain for challenges). If multiple vhosts are
        found matching this criteria, an error will be raised because we don't know which one should have TLS enabled by default in nginx configuration files
        generated by Certbot's setup function (see `setup` documentation).
        """
    ...
    def _set_up_challenges(self):
        """
        .. automethod :: acme_tiny.HTTP01._set_up_challenges
           :members:
           :undoc-members:
        """
    ...
    def _set_up_challenge(self, achall):
        """
        Sets up a challenge for the specified :class:`letsencrypt.achallenges.AnnotatedChallenge`.

        Creates a file in the temporary storage directory with the
        specified name and contents, and sets its access mode to 0644 (octal). Returns an ACME response object that can be returned from a `http-01` or
        `dns-01` plugin's :func:`~letsencrypt_apache.configurator.ApacheConfigurator.setup_challenge`.
        """
