...
from certbot_integration_tests.utils.misc import skip_on_pebble, skip_on_pebble_strict
...
def test_hook_dirs_creation(common, renewal_hooks_dirs):
    common(['register'])
...
def test_renew(config_dir, common_no_force_renew, common, monkeypatch):
    ...
    common(['renew', '--no-renew-time-shuffle',
            '--cert-name', certname, '--authenticator', 'manual'])
