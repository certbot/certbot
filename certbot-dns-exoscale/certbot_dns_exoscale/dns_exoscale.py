...
ACCOUNT_URL = "https://portal.exoscale.com/"
...
class Authenticator(dns_common.DNSAuthenticator):
    ...
    def add_parser_arguments(cls, add):  # pylint: disable=arguments-differ
        ...
        add("credentials", help="Exoscale credentials INI configuration file.")
...
class _ExoscaleLexiconClient(dns_common_lexicon.LexiconClient):
    ...
    def __init__(self, key, secret, ttl):
        ...
        config = config.with_env().with_dict(
            {
                "provider": "exoscale",
                "exoscale": {"auth_key": key, "auth_secret": secret},
                "ttl": ttl,
            }
        )

        self.provider = exoscale.Provider(config)
