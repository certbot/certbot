...
class Authenticator(dns_common.DNSAuthenticator):
    ...
    description = 'Obtain certificates using a DNS TXT record (if you are using Plesk for DNS).'
    ...
    def _setup_credentials(self):
        self.credentials = self._configure_credentials(
            ...
            {
                'username': 'Username for Plesk API. (See {0}.)',
                'password': 'Password for Plesk API. (See {0}.)',
                'plesk_server': 'Url of the host https://hostname:port',
            }
        )
    ...
    def _get_plesk_client(self,domain):
        return _PleskLexiconClient(self.credentials.conf('plesk_server'), 
                 self.credentials.conf('auth_username'),
                 self.credentials.conf('auth_password'),
                 domain)
...
class _PleskLexiconClient(dns_common_lexicon.LexiconClient):
    ...
    def __init__(self, ples_server, username, password, domain ):
    ...
    def _handle_http_error(self, e, domain_name):
        ...
        if str(e).startswith('401 Client Error: Unauthorized for url:'):
            hint = 'Is your plesk_server & authentication info correct?'
