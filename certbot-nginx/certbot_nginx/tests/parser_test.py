...
class NginxParserTest(util.NginxTest): #pylint: disable=too-many-public-methods
    ...
    def test_load(self):
        ...
        self.assertEqual([['server_name', 'somename', 'alias', 'another.alias']],
                         nparser.parsed[nparser.abs_path('server.conf')].dump())
        self.assertEqual([[['server'], [['listen', '69.50.225.155:9000'],
                                        ['listen', '127.0.0.1'],
                                        ['server_name', '.example.com'],
                                        ['server_name', 'example.*']]]],
                         nparser.parsed[nparser.abs_path(
                             'sites-enabled/example.com')].dump())
    ...
    def test_filedump(self):
        ...
        parsed = nginx_obj.parse_from_file_nginx(
                     nginx_obj.NginxParseContext(self.config_path, nparser.abs_path(
    ...
    def test_filedump_whitespace(self):
        nparser = parser.NginxParser(self.config_path, 'funky_whitespace.conf')
        nparser.filedump('test', lazy=False)
    ...
    def test_has_ssl_on_directive(self):
        nparser = parser.NginxParser(self.config_path)
        fake_bloc = nginx_obj.parse_raw_nginx([['server'],
    ...
    def test_add_server_directives(self):
        ...
        dump = nginxparser.dumps_raw(
            nparser.parsed[nparser.abs_path('nginx.conf')].dump(include_spaces=True))
        ...
        self.assertEqual(nparser.parsed[nparser.abs_path('sites-enabled/example.com')].dump(),
            [[['server'], [['listen', '69.50.225.155:9000'],
                           ...
                           ['server_name', '.example.com'],
                           ['server_name', 'example.*'],
                           ['foo', 'bar'], ['#', parser_obj.COMMENT],
                           ['ssl_certificate', '/etc/ssl/cert2.pem'], ['#', parser_obj.COMMENT]]]])
        # Ensure adding a unique non-repeatable directive raises error
        from certbot import errors
        self.assertRaises(errors.MisconfigurationError,
            nparser.add_server_directives, example_com,
            [['foo', 'bar'], ['ssl_certificate', '/etc/ssl/cert_new.pem']])
    ...
    def test_replace_server_directives(self):
        ...
        self.assertEqual(
            ...
            [[['server'], [['listen', '69.50.225.155:9000'],
                           ...
                           ['#', parser_obj.COMMENT],
                           ['server_name', 'example.*'],
                           ['ssl_certificate', 'cert.pem'], ['#', parser_obj.COMMENT]]]])
