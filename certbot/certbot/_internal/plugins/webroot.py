            ...
            if not filesystem.POSIX_MODE:
                ...
                if os.path.exists(web_config_path):
                    logger.info("A web.config file has not been created in "
                                f"{self.full_roots[name]} because another one already exists.")
                    return
                logger.info(f"Create a web.config file in {self.full_roots[name]} to allow IIS "
                            "to serve challenge files.")
            ...
            if root_path is not None:
                ...
                if os.path.exists(web_config_path):
                    ...
                    if sha256sum in _WEB_CONFIG_SHA256SUMS:
                        ...
                        logger.info(f"Not cleaning up the web.config file in {root_path} "
