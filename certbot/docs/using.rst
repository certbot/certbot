...
Under Windows, Certbot will generate a ``web.config`` file in ``/.well-known/acme-challenge``
in order to let IIS serve the challenge files even if they do not have an extension.
