...
class BaseCertManagerTest(test_util.ConfigTestCase):
    ...
    def setUp(self):
        ...
        security.makedirs(self.config.renewal_configs_dir)
    ...
    def _set_up_config(self, domain, custom_archive):
        ...
        security.makedirs(os.path.join(self.config.live_dir, domain))
        config_file = configobj.ConfigObj()

        if custom_archive is not None:
            security.makedirs(custom_archive)
            config_file["archive_dir"] = custom_archive
        else:
            security.makedirs(os.path.join(self.config.default_archive_dir, domain))
...
class CertificatesTest(BaseCertManagerTest):
    ...
    def test_certificates_no_files(self, mock_utility, mock_logger):
        ...
        security.makedirs(empty_config.renewal_configs_dir)
