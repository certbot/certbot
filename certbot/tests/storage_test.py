...
CERT = util.load_cert('cert_512.pem')
...
class RenewableCertTests(BaseRenewableCertTest):
    ...
    def test_names(self):
        ...
        self._write_out_kind("cert", 12, util.load_vector("cert-san_512.pem"))
        self.assertEqual(self.test_rc.names(),
                         ["example.com", "www.example.com"])

        # Trying a non-current version
        self._write_out_kind("cert", 15, util.load_vector("cert_512.pem"))
        self.assertEqual(self.test_rc.names(12),
                         ["example.com", "www.example.com"])

        # Testing common name is listed first
        self._write_out_kind(
            "cert", 12, util.load_vector("cert-5sans_512.pem"))
        self.assertEqual(
    ...
    def test_time_interval_judgments(self, mock_datetime):
        ...
        test_cert = util.load_vector("cert_512.pem")
        self._write_out_ex_kinds()
