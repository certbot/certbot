...
def _sync_candidate_from_temp_to_origin(version: str) -> None:
    message = (f'Error running `git pull temp candidate-{version}`. To run successfully, '
                'stash any changes you\'ve made to this branch. Do not attempt to merge and '
                'continue, as that will fail.')
    _run_silent_except_error(f'git pull temp candidate-{version}'.split(), message)
    _run_silent_except_error(f'git push origin candidate-{version}'.split())
