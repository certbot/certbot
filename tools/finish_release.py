...
def _sync_candidate_from_temp_to_origin(version: str) -> None:
    ...
    try:
        ...
        print(f'Error running `git pull temp candidate-{version}`. Before rerunning this script, '
               'run `git log` to make sure the most recent commit is bumping the version number, '
               'or the branch without the version number bump will be created incorrectly.')
        print(e.stderr)
        raise e
...
def _run_allow_exists(cmd: list[str], description: str) -> subprocess.CompletedProcess:
    try:
...
def _create_release_pr_to_main(version: str) -> None:
    ...
    proc = _run_allow_exists(cmd, 'PR to merge release changes into main')
    print(f'Created: {proc.stdout.rstrip()}')
...
def _create_release_pr_to_minor_branch(
    ...
    proc = _run_allow_exists(cmd, 'PR to merge release changes into .x branch')
    print(f'Created: {proc.stdout.rstrip()}')
